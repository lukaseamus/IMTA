##### Project: Review of literature on plant-based integrated multitrophic aquaculture (IMTA)
...
#### Figure 1. Literature dissemination
### Citations
## Load data
...
seaweeds.plot <- ggplot(data = seaweeds, aes(fct_reorder(seaweed, papers))) +
                    ...
                    geom_point(aes(y = production/c2), colour = "#5b5b5e", fill = "#bdd268", size = 6, shape = 21) +
                    labs(y = expression("Papers")) +
                    ...
                    scale_y_continuous(breaks = seq(0, 40, by = 10),
                                       sec.axis = sec_axis(~.*c2, name = expression("Global aquaculture production (t yr"^-1*")"),
